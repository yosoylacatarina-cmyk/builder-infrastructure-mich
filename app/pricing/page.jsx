...
import { useState, useEffect } from 'react'import { createClient } from '@supabase/supabase-js'

const supabase = createClient(process.env.NEXT_PUBLIC_SUPABASE_URL,process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY)

const assumptions = [{ tier: 'Free', price: '$0/mo', description: 'Basic routine generator only' },{ tier: 'Freemium', price: '$5/mo', description: 'Save routines + AI personalization' },{ tier: 'Pro', price: '$15/mo', description: 'All features + priority support' },]

export default function PricingPage() {const [freeUsers, setFreeUsers] = useState(100)const [freemiumUsers, setFreemiumUsers] = useState(50)const [proUsers, setProUsers] = useState(20)const [isAnnual, setIsAnnual] = useState(false)const [scenarioName, setScenarioName] = useState('')const [savedScenarios, setSavedScenarios] = useState([])const [saving, setSaving] = useState(false)const [saveMsg, setSaveMsg] = useState('')

const monthlyRevenue = (freemiumUsers * 5) + (proUsers * 15)const annualRevenue = monthlyRevenue * 12const displayRevenue = isAnnual ? annualRevenue : monthlyRevenueconst totalUsers = Number(freeUsers) + Number(freemiumUsers) + Number(proUsers)const conversionRate = totalUsers > 0 ? (((Number(freemiumUsers) + Number(proUsers)) / totalUsers) * 100).toFixed(1) : 0

useEffect(() => { fetchScenarios() }, [])

async function fetchScenarios() {const { data } = await supabase.from('pricing_scenarios').select('*').order('created_at', { ascending: false })if (data) setSavedScenarios(data)}

async function handleSave() {if (!scenarioName.trim()) { setSaveMsg('Please enter a scenario name.'); return }setSaving(true)setSaveMsg('')const { error } = await supabase.from('pricing_scenarios').insert([{name: scenarioName,free_users: Number(freeUsers),freemium_users: Number(freemiumUsers),pro_users: Number(proUsers),monthly_revenue: monthlyRevenue,}])if (error) {setSaveMsg('Error saving. Check Supabase connection.')} else {setSaveMsg('Saved!')setScenarioName('')fetchScenarios()}setSaving(false)}

return (Gym Routine GeneratorHomeCoreResearchProductPricingDocs
